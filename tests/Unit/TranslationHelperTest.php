...
it('applies custom fallback strategy from configuration', function () {
    Config::set('filament-auto-translation.fallback_strategies.custom_test', fn ($key) => 'CUSTOM_' . strtoupper($key));
    Config::set('filament-auto-translation.components.resource_labels.fallback_strategy', 'custom_test');
...
it('applies default fallback strategy when configured strategy is not callable', function () {
    Config::set('filament-auto-translation.fallback_strategies.invalid_strategy', 'not_callable');
    Config::set('filament-auto-translation.components.resource_labels.fallback_strategy', 'invalid_strategy');
...
it('applies match statement fallback strategies', function () {
    ...
    Config::set('filament-auto-translation.components.resource_labels.fallback_strategy', 'unknown_strategy');
...
it('logs missing translation when debug is enabled', function () {
    Config::set('filament-auto-translation.debug.log_missing_translations', true);

    Log::shouldReceive('info')
        ->once()
        ->with('Filament Auto Translation: Missing translation', [
            'key' => 'missing_key',
...
it('does not log missing translation when debug is disabled', function () {
    Config::set('filament-auto-translation.debug.log_missing_translations', false);
...
it('handles component configuration when component is enabled but no fallback strategy defined', function () {
    Config::set('filament-auto-translation.components.test_component', ['enabled' => true]);
...
it('uses title_case fallback strategy from match statement', function () {
    Config::set('filament-auto-translation.components.resource_labels.fallback_strategy', 'title_case');
    Config::set('filament-auto-translation.fallback_strategies', []); // Empty to force match statement
